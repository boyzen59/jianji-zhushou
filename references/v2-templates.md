...
5. 按用户要求的格式生成审阅；需要PPT时用原生可编辑对象，需要PDF时保留文字和独立图片，不能把静态预览称为动画。

内容约束见content-and-approval.md，视觉见visual-style.md；不把原模板主题、示例值或入场方式一并继承。
...
- 展示系统导航、页脚B-ROLL、FRAME/BUILD等样张元信息不导入；其余标签范围遵从content-and-approval.md。
